...
def get_system_health():
    ...
    try:
        ...
        redis_status  = "ONLINE"
        redis_color   = "#c9a84c"
        redis_mem     = round(ri.get('used_memory', 0)/(1024*1024), 2)
        redis_keys    = ri.get('db0', {{}}).get('keys', 0) if 'db0' in ri else 0
        redis_clients = ri.get('connected_clients', 1)
    except:
        redis_status  = "OFFLINE"
        redis_color   = "#e05c5c"
        redis_mem = redis_keys = redis_clients = 0
    ...
    return dict(
        ...
        redis_mem=redis_mem, redis_keys=redis_keys, redis_clients=redis_clients,
    )
...
<head>
    ...
    <style>
        *, *::before, *::after {{ box-sizing:border-box; margin:0; padding:0; }}
        ...
        /* Rotating ring graphic */
        .ring {{
            position:fixed; border-radius:50%; pointer-events:none; z-index:0;
            border:1px solid rgba(201,168,76,0.07);
        }}
        .ring-1 {{ width:600px; height:600px; top:50%; left:50%; transform:translate(-50%,-50%); animation:spin1 40s linear infinite; }}
        .ring-2 {{ width:850px; height:850px; top:50%; left:50%; transform:translate(-50%,-50%); border-style:dashed; animation:spin1 60s linear infinite reverse; }}
        .ring-3 {{ width:1100px; height:1100px; top:50%; left:50%; transform:translate(-50%,-50%); animation:spin1 80s linear infinite; }}
        ...
        .topbar {{
            ...
            border-bottom:1px solid var(--gold-border); padding-bottom:10px;
        ...
        .top-title {{ font-family:'Bebas Neue',cursive; font-size:0.9rem; font-weight:700; letter-spacing:2px; color:var(--gold-light); }}
        ...
        .hero-eyebrow {{
            ...
            font-size:0.6rem; letter-spacing:7px;
        ...
        .hero-typewriter {{
            ...
            animation:typing 2.5s steps(28,end) 1s forwards, blink 0.8s step-end infinite;
            max-width:0;
        }}
        ...
        .hero-desc {{
            font-size:0.78rem; font-weight:300; letter-spacing:1.5px;
            color:var(--dim); max-width:520px; line-height:1.6;
        ...
        .kpi-value {{ font-family:'Bebas Neue',cursive; font-size:1.5rem; font-weight:700; color:var(--gold); text-shadow:0 0 14px rgba(201,168,76,0.5); }}
        ...
        .hcard-val   {{ font-family:'Bebas Neue',cursive; font-size:0.9rem; font-weight:600; line-height:1; margin-bottom:4px; transition:color 0.4s; pointer-events:none; }}
        ...
        .modal {{
            background:linear-gradient(145deg,#071428 0%,#040c18 100%);
            border:1px solid var(--gold-border); border-radius:18px;
            padding:30px 34px; min-width:340px; max-width:440px; width:92%;
            position:relative;
            box-shadow:0 0 60px rgba(201,168,76,0.12), inset 0 1px 0 rgba(201,168,76,0.1);
        ...
        .modal-title  {{ font-family:'Bebas Neue',cursive; font-size:1rem; font-weight:700; color:var(--gold); letter-spacing:2px; }}
        .modal-status {{ font-family:'Share Tech Mono',monospace; font-size:0.55rem; letter-spacing:3px; color:var(--dim); margin-top:3px; }}
        .modal-big {{ font-family:'Bebas Neue',cursive; font-size:3.2rem; font-weight:700; line-height:1; text-align:center; margin:16px 0; }}
        ...
        .mdet-val   {{ font-family:'Bebas Neue',cursive; font-size:0.95rem; font-weight:600; color:var(--gold-light); }}
...
<body>
    ...
    <div class="dashboard">
        ...
        <div class="health-section">
            ...
            <div class="health-grid">
                ...
                <div class="hcard" onclick="openModal('redis')">
                    <div class="hcard-icon">🗄️</div>
    ...
    <script>
        ...
        let hd = {{
            ...
            redis_status:"{h['redis_status']}", redis_color:"{h['redis_color']}",
            redis_mem:{h['redis_mem']}, redis_keys:{h['redis_keys']}, redis_clients:{h['redis_clients']},
        }};
        ...
        const defs = {{
            ...
            redis:  d=>({{ icon:'🗄️', title:'REDIS CACHE',  status:`Cache Layer · ${{d.redis_status}}`,              big:d.redis_status,  color:d.redis_color, pct:d.redis_status==='ONLINE'?100:0, details:[['Status',d.redis_status],['Memory',`${{d.redis_mem}} MB`],['Keys',d.redis_keys],['Clients',d.redis_clients]] }}),
        ...
        function refreshHealth() {{
            ...
            fetch('/health').then(r=>r.json()).then(d=>{{
                ...
                const cards=[
                    ...
                    {{id:'redis', val:d.redis_status,   color:d.redis_color, badge:d.redis_status, pct:d.redis_status==='ONLINE'?100:0, fs:'0.65rem'}},
        ...
        // Theme palettes — background click only
        const palettes=[
